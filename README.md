# 🚀 Blockchain Journey

I am learning Blockchain and Web3 development step by step.

## 📅 Day 1
- Learned basics of blockchain
- Understood decentralization and hashing
- Set up MetaMask wallet
- Learned about private key and seed phrase security

## 🧠 Key Learnings
- Blockchain is decentralized (no single authority)
- Hash ensures data integrity
- Private key = full ownership of wallet

## 🎯 Goal
To become a Blockchain Developer.

## 📌 Upcoming Plan
- Learn Solidity
- Build smart contracts
- Create DApps

## 🛠️ Tech Stack (learning)
- Solidity
- Ethereum
- JavaScript


## 📅 Day 2
- Learned Solidity basics
- Created first smart contract
- Understood state variables and functions
- Built a Sample_Identity contract on Remix IDE
  
